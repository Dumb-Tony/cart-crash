...
function slalomInput(s){const gate=g.GATES.find(x=>x.s>s.s)||{x:0};return {...neutral,steer:Math.abs(gate.x-s.x)>.5?Math.sign(gate.x-s.x):0}}
let slalomSummary=[];for(const fps of [30,60,120]){const sl=g.fresh();sl.contract='slalom';while(sl.s<g.LENGTH){for(let i=0;i<120/fps&&sl.s<g.LENGTH;i++)g.step(sl,slalomInput(sl))}assert.equal(sl.gateHits,6);assert(g.contractWon(sl));assert.equal(sl.crashes,0);slalomSummary.push({fps,time:sl.time,score:sl.bank});const hits=sl.gateHits;sl.s=179.99;g.step(sl,neutral);assert.equal(sl.gateHits,hits,'gate cannot be farmed')}
assert(slalomSummary.every(r=>r.time===slalomSummary[0].time&&r.score===slalomSummary[0].score));console.log('Slalom:',JSON.stringify(slalomSummary));
const missed=g.fresh();missed.contract='slalom';while(missed.s<g.LENGTH)g.step(missed,neutral);assert(!g.contractWon(missed));
const failedAir=g.fresh();Object.assign(failedAir,{contract:'air',s:g.LENGTH,gaps:1,clean:1,crashes:1});assert(!g.contractWon(failedAir));failedAir.crashes=0;assert(g.contractWon(failedAir));
const trophy=boot();trophy.g.restart();Object.assign(trophy.g.state,{s:1799.99,time:30});trophy.g.frame(1);trophy.g.frame(34);assert(trophy.persisted.badges.includes('dash'));assert(trophy.persisted.records.dash.time>30);const trophyReload=boot(false,trophy.persisted);assert(trophyReload.els.panel.innerHTML.includes('★ Downhill dash'));trophyReload.els.reset.onclick();assert.equal(trophyReload.persisted.badges.length,0);
// Fast charge and forgiving ramp timing: an approach hop must not suppress the lip.
const charged=g.fresh();for(let n=0;n<30;n++)g.step(charged,{...neutral,crouch:true});assert(charged.charge>.99,'full charge in 0.25 seconds');
const highArc=g.fresh();Object.assign(highArc,{s:899.9,x:4,h:6,vy:2});g.step(highArc,neutral);assert(highArc.vy<2,'ramp must not relaunch an already high arc');
let rampCases=0;for(const speed of [30,42,58])for(const release of [860,875,885,895,899,902,null])for(const lane of [2,4,7]){const r=g.fresh();Object.assign(r,{s:840,x:lane,v:speed,safeS:840,safeX:lane});let launched=false;while(r.s<1050&&r.time<15&&!r.crashes){g.step(r,{...neutral,crouch:release===null||r.s<release});if(r.s>=900&&r.s<902&&r.vy>10)launched=true}assert(launched,`ramp failed at ${speed}/${release}/${lane}`);assert.equal(r.crashes,0,`ramp crash at ${speed}/${release}/${lane}`);assert.equal(r.gaps,1);assert.equal(r.clean,1);rampCases++}console.log('Ramp timing/lanes:',rampCases,'passed');
